...
export default class RunsTrigger extends DashboardCommand {
	...
	static override flags = {
		...
		'work-item-url': Flags.string({ description: 'Work item URL (optional)' }),
		'work-item-title': Flags.string({ description: 'Work item title (optional)' }),
		...
		try {
			const result = await this.withSpinner('Triggering agent run...', () =>
				this.client.runs.trigger.mutate({
					...
					workItemUrl: flags['work-item-url'],
					workItemTitle: flags['work-item-title'],
