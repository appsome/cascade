...
export class GitLabRouterAdapter implements RouterPlatformAdapter {
	...
	async dispatchWithCredentials(
		...
		const gitlabToken = await getIntegrationCredential(fullProject.id, 'scm', 'implementer_token');
	...
	async postAck(
		...
		try {
			...
			let token: string;
			try {
				token = await getIntegrationCredential(project.id, 'scm', 'implementer_token');
			} catch {
