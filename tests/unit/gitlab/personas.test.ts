...
describe('GitLab personas', () => {
	...
	describe('getPersonaToken', () => {
		it('resolves implementer_token for implementer agents', async () => {
			...
			expect(getIntegrationCredential).toHaveBeenCalledWith('proj-1', 'scm', 'implementer_token');
		...
		it('resolves reviewer_token for review agent', async () => {
			...
			expect(getIntegrationCredential).toHaveBeenCalledWith('proj-1', 'scm', 'reviewer_token');
