...
| `org_memberships` | Multi-org membership: links a user to an org with a per-org role, so one account can belong to many orgs. `users.org_id`/`users.role` remain the home org + global role. Ships dormant (spec 021 plan 1) — read by plan 2. | UNIQUE(`user_id`, `org_id`) |
