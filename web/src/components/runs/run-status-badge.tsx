...
const statusStyles: Record<string, string> = {
	// MNG-1695: `queued` is the pre-dispatch state (yellow, distinct from
	// `timed_out`'s amber) before the worker boots and flips it to `running`.
	...
	return (
		<span
			className={cn(
				...
				statusStyles[status] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-100',
			)}
		>
