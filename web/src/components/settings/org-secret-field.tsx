/**
 * Reusable org-scoped secret input field.
 * Write-only — shows masked metadata when configured, never exposes plaintext.
 * Sibling of ProjectSecretField (deliberately not generalized: that component
 * carries project-only verify plumbing across six call sites).
 */
...
export function OrgSecretField({
	envVarKey,
	label,
	description,
	placeholder,
	...
	const invalidate = () =>
		queryClient.invalidateQueries({
			queryKey: trpc.organization.credentials.list.queryOptions().queryKey,
		});

	const saveMutation = useMutation({
		mutationFn: () =>
			trpcClient.organization.credentials.set.mutate({
				envVarKey,
				value,
				name: label,
			}),
	...
	const deleteMutation = useMutation({
		mutationFn: () => trpcClient.organization.credentials.delete.mutate({ envVarKey }),
		onSuccess: () => {
			invalidate();
		},
	});
	...
	return (
		<div className="space-y-2">
			<div className="flex items-center gap-2">
				<Label htmlFor={`org-secret-${envVarKey}`}>{label}</Label>
