...
## Named credential sets + engine-credential rotation

Engine and SCM providers (**anthropic, openai, openrouter, github, gitlab** — catalog: `CREDENTIAL_PROVIDERS` in `src/config/credentialProviders.ts`, imported by backend AND web) support **multiple named org credentials** since migration 0063. `org_credentials` stays the single encrypted value store (AAD = orgId); a row either belongs to a named `org_credential_sets` entry (`set_id`) or stays a flat base-tier row (`set_id IS NULL` — PM/alerting/custom keys, exactly the pre-0063 model). Projects pick sets via `project_credential_selections` (single row per provider; **ordered rows for anthropic** — the rotation pool, position 0 = primary). **Resolution precedence per key:** project_credentials override > selected set (lowest position) > org default set > base-tier row — all inside `resolveProjectCredential` / `resolveAllProjectCredentials` (`src/db/repositories/credentialsRepository.ts`), so every consumer (worker env, personas, discovery) inherits it. `writeOrgCredential` reroutes provider-mapped keys into the provider's default set (legacy CLI/tRPC callers stay coherent — a flat base row for those keys would be shadowed). Exactly one default set per (org, provider); org set delete is blocked while projects reference it (`force: true` cascades). UI: org credentials page is per-provider tabs; project settings render `ProjectCredentialSelector` per provider (harness engine tabs, SCM tab, General → OpenRouter).

**Rotation (claude-code engine only).** At dispatch, `spawnWorker` calls `resolveClaudeCredentialForJob` (`src/router/engine-credential-rotation.ts`): pool from `resolveCredentialPool(projectId, 'anthropic')` (a project-local override short-circuits to a one-member pool), limits per token from the Anthropic usage API (`src/anthropic/client.ts` — parser preserves machine-readable bucket **keys** and passes unknown buckets through, so a future model-class window works untouched), score = max utilization over the run model's **gating buckets** (`src/anthropic/bucket-matching.ts`: `five_hour`/`seven_day` exact-match globals + family-token buckets like `seven_day_opus`/`seven_day_sonnet`/future `seven_day_fable`; oauth_apps/cowork/unknown non-family keys never gate). Least-utilized under `ROTATION_UTILIZATION_THRESHOLD` (default 95) wins; ties → pool order; limits-fetch failure → **never suspend**, fall back to pool order. The picked token replaces every other `CLAUDE_CODE_OAUTH_TOKEN` source in the worker env (precedence: pool > DB credential > **host env fallback** — the router host token is fallback-only since the rotation work; it previously silently overrode DB credentials via Docker last-wins) plus `CASCADE_ENGINE_CREDENTIAL_ID/NAME` attribution vars, recorded on `agent_runs.engine_credential_id/name` (no FK — history survives set deletion). Concurrent same-token picks are accepted (eventually-consistent utilization; load-spreading, not reservation).

**Suspension.** When every pool candidate with known limits is at/over the threshold: the run flips to **`status='suspended'`** (`resume_at` + reason on the row), the BullMQ job **completes** (locks released explicitly via `releaseLocksForFailedJob` — the failed-event compensation never fires), and a delayed `resume-suspended-run` job on `cascade-dashboard-jobs` (deliberately NOT a delayed re-add on `cascade-jobs` — coalesce-key scanning would supersede it) fires at the earliest gating-bucket reset (clamped 1min–8d, +jitter; 15min fallback). Resume (`src/router/run-suspension.ts`): status guard (cancel-during-suspension self-neutralizes) → supersede guard (newer run for the work item wins) → fresh limit re-check (still exhausted → re-suspend, flap canary at 10) → requeue `suspended → queued` → re-submit the original job with the run id threaded (`runId` for manual-run, `preCreatedRunId` via `triggerResult.agentInput` for webhook jobs and as a job field for retry-run/debug-analysis). **`'suspended'` is deliberately NOT in `ACTIVE_RUN_STATUSES`** — capacity math, orphan cleanup, and worker-exit reconciliation must ignore it (no worker exists); only the duplicate-dispatch guards (`hasActiveRunForWorkItem`, work-item lock) opt in via `includeSuspended`. Suspended runs are cancellable (no container — no cancel command published) and not retryable (auto-resume would duplicate).
